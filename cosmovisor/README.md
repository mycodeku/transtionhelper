...
```sh
go install github.com/mycodeku/transtionhelper/cosmovisor/cmd/cosmovisor@latest
```
...
```sh
go install github.com/mycodeku/transtionhelper/cosmovisor/cmd/cosmovisor@v0.1.0
```
...
Next, we can add a migration - which is defined using `x/upgrade` [upgrade plan](https://github.com/mycodeku/transtionhelper/blob/master/docs/core/upgrade.md) (you may refer to a past version if you are using an older Cosmos SDK release). In a migration we can do any deterministic state change. 
