...
The Cosmos SDK maintains a set of [Protobuf definitions](https://github.com/mycodeku/transtionhelper/tree/master/proto/cosmos). It is important to correctly design Protobuf definitions to avoid any breaking changes within the same version. The reasons are to not break tooling (including indexers and explorers), wallets and other third-party integrations.
...
* Marking fields as `reserved`. Protobuf proposes the `reserved` keyword for removing fields without the need to bump the package version. However, by doing so, client backwards compatibility is broken as Protobuf doesn't generate anything for `reserved` fields. See [#9446](https://github.com/mycodeku/transtionhelper/issues/9446) for more details on this issue.
...
When processing `Msg`s, the Cosmos SDK's antehandlers are strict and don't allow unknown fields in `Msg`s. This is checked by the unknown field rejection in the [`codec/unknownproto` package](https://github.com/mycodeku/transtionhelper/blob/master/codec/unknownproto).
...
* The Cosmos SDK recently removed support for [time-based software upgrades](https://github.com/mycodeku/transtionhelper/pull/8849). As such, the `time` field has been marked as deprecated in `cosmos.upgrade.v1beta1.Plan`. Moreover, the node will reject any proposal containing an upgrade Plan whose `time` field is non-empty.
...
* [#9445](https://github.com/mycodeku/transtionhelper/issues/9445) Release proto definitions v1
* [#9446](https://github.com/mycodeku/transtionhelper/issues/9446) Address v1beta1 proto breaking changes
